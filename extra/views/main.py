...
from mrwf.extra.views import abook
...
@login_required
def search (request):
    ctx = abook.SearchHelper (request)
    tpl_vars = {}
    ...
    if ctx.match:
        ...
        if ctx.opt_contacts:
            ...
            contacts = list (abook.search_contacts (keywords))
            for c in contacts:
                obj_id = c.obj_id
                ...
                if obj.type == Contactable.ORGANISATION:
                    ...
                    if p:
                        abook.append_person (people, p, (c,))
        else:
            p_list = list (abook.search_people (keywords))[:30]
            o_list = list (abook.search_orgs (keywords))[:30]
